...
import { 
  Layout, Settings, Activity, Monitor, Box, Server, Terminal, 
  BrainCircuit, Loader2, Play, Pause, Maximize, Minimize,
  ShoppingCart, RotateCcw, Truck, TrendingDown, Thermometer, Wind, Users, Cpu, ChevronRight,
  BarChart3, Zap, Circle, LayoutDashboard // Added new icons
} from 'lucide-react';

// Custom Components
import Smart_Logistics_Rail_dashboard from './Smart_Logistics_Rail_dashboard';
...
const MainContent = styled.main`
  ...
  padding: 40px;
  background-image: 
    radial-gradient(circle at top right, #1e293b 0%, transparent 45%),
...
// 리스트 뷰 전용 스타일
const StatusTable = styled.div` width: 100%; display: flex; flex-direction: column; gap: 12px; `;
const ZoneRow = styled.div`
  display: grid; grid-template-columns: 0.8fr 1.5fr 1.5fr 1fr 1fr 0.5fr;
  align-items: center; background: #0b0e14; border: 1px solid #1e293b;
  border-radius: 16px; padding: 16px 24px; cursor: pointer;
  &:hover { border-color: #3b82f6; }
`;
const LoadBar = styled.div`
  width: 100px; height: 6px; background: #1e293b; border-radius: 3px;
  position: relative; overflow: hidden;
  &::after {
    content: ''; position: absolute; left: 0; top: 0; height: 100%;
    width: ${props => props.val}%; background: ${props => props.val > 80 ? '#ef4444' : '#3b82f6'};
  }
`;
...
const SmartLogisticsDashboard = () => {
  // 현재 활성화된 탭을 관리하는 상태 (초기값: 'rail_dashboard')
  const [currentTab, setCurrentTab] = useState('rail_dashboard'); 
  // 선택된 구역의 ID를 관리하는 상태
  ...
  return (
    ...
    <Container isFullscreen={isPseudoFullscreen}>
      ...
      <Sidebar>
        {/* 로고 버튼 */}
        <NavLogo onClick={() => setCurrentTab('macro')}><Monitor size={28} color="white"/></NavLogo>
        {/* 내비게이션 버튼들 */}
        <NavButton active={currentTab === 'macro'} onClick={() => setCurrentTab('macro')}><Layout size={24}/></NavButton>
        <NavButton active={currentTab === 'micro'} onClick={() => setCurrentTab('micro')}><Activity size={24}/></NavButton>
        <NavButton active={currentTab === 'rail_dashboard'} onClick={() => setCurrentTab('rail_dashboard')}><Server size={24}/></NavButton> {/* New button */}
        <NavButton active={currentTab === 'editor'} onClick={() => setCurrentTab('editor')}><Box size={24}/></NavButton>
        <NavButton active={currentTab === 'settings'} onClick={() => setCurrentTab('settings')}><Settings size={24}/></NavButton>

      </Sidebar>

      {/* 메인 콘텐츠 영역 */}
      <MainContent>
        {/* rail_dashboard 탭이 아닐 경우에만 헤더를 표시 */}
        {currentTab !== 'rail_dashboard' && (
          <Header>
            <HeaderTitleGroup>
        ...
        {currentTab === 'macro' && (
          <>
              ...
              {[
                ...
                <KpiCard key={i}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ fontSize: '10px', fontWeight: 900, color: '#64748b' }}>{kpi.label}</span>
            ...
            <AiBanner>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '13px', color: '#cbd5e1', fontWeight: 'bold', margin: '4px 0 0' }}>{aiInsight || "운영 전략 실시간 분석 대기 중..."}</p>
                </div>
              </div>
            ...
            {currentData.zones.map(zone => (
              <ZoneRow key={zone.id} onClick={() => { setSelectedZoneId(zone.id); setCurrentTab('analytics'); }}>
                <span style={{ color: '#3b82f6', fontWeight: 900 }}>{zone.id}</span>
                <span style={{ fontWeight: 800 }}>{zone.name}</span>
                <div>
                  <div style={{ fontSize: '12px', marginBottom: '4px' }}>{zone.load}%</div>
                  <LoadBar val={zone.load} />
                </div>
                <span style={{ fontWeight: 900, color: zone.temp > 60 ? '#ef4444' : '#fff' }}>{zone.temp}°C</span>
            ...
            {/* 데이터 재생 컨트롤 바 */}
            <PlaybackBar>
              <button onClick={() => setIsPlaybackMode(!isPlaybackMode)} style={{ background: isPlaybackMode ? '#f59e0b' : '#1e293b', border: 'none', padding: '14px', borderRadius: '12px', color: 'white', cursor: 'pointer' }}>
        ...
        {/* 'micro' 탭 내용: MicroPage 컴포넌트 렌더링 */}
        {currentTab === 'micro' && (<MicroPage />)}

        {/* 'rail_dashboard' 탭 내용: Smart_Logistics_Rail_dashboard 컴포넌트 렌더링 */}
        {currentTab === 'rail_dashboard' && <Smart_Logistics_Rail_dashboard />}
      </MainContent>
    </Container>
  );
};
