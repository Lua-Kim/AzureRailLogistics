...
### 1. 전체 아키텍처 흐름: 메달리온(Medallion) & 람다(Lambda) 하이브리드

초당 1만 건의 데이터는 '속도'와 '정확도'를 모두 잡아야 합니다. 이를 위해 데이터를 3단계 품질로 나누는 **메달리온 아키텍처**와 실시간/배치를 나누는 **람다 아키텍처**를 결합하는 것이 좋습니다.

#### ① Ingestion Layer (수집층)

* **Azure Event Hubs (Kafka 호환):** 로컬 카프카의 클라우드 확장판입니다. 초당 1만 건 이상의 대량 유입을 버티는 완충 지대 역할을 합니다.

#### ② Hot Path (실시간/운영층) - "지금 무슨 일이 일어나는가?"

* **Azure Redis Cache (추천):** 대시보드의 실시간 KPI(병목 현상, 현재 가동률)는 DB를 매번 조회하면 성능이 떨어집니다. 가장 최신 데이터는 메모리 기반인 Redis에 두고 대시보드가 여기서 바로 읽게 합니다.

* **Azure SQL Database:** 회원 정보, 물류센터 마스터 정보(구역 이름, 레일 개수 등), 운영 설정값. 데이터의 **정합성**이 가장 중요한 정보를 담습니다.

#### ③ Cold/Warm Path (분석/이력층) - "왜 이런 일이 일어났는가?"

* **Azure Fabric (KQL Database/Real-Time Intelligence):** 센서 원본 데이터를 즉시 분석하기 위해 SQL 대신 KQL(Kusto Query Language) 기반 데이터베이스를 사용합니다. 초당 수만 건의 로그성 데이터를 밀리초 단위로 검색하는 데 최적입니다.

* **Azure Fabric (OneLake):** 모든 원천 데이터는 여기에 **메달리온 구조**로 저장됩니다.
  
  * **Bronze:** 센서 원본 (Event Hubs에서 그대로 저장)
  
  * **Silver:** 결측치 제거, SQL의 마스터 정보와 결합(Join)된 데이터
  
  * **Gold:** AI 모델용 데이터, 주간/월간 손실 리포트용 집계 데이터

* * *

### 2. 저장소별 상세 데이터 매핑 전략

단순히 "SQL은 설정, Fabric은 센서"를 넘어 아키텍처적 깊이를 더해봅니다.

| **분류**      | **저장소**              | **데이터 종류 (예시)**                       | **아키텍처적 이유**                   |
| ----------- | -------------------- | ------------------------------------- | ------------------------------ |
| **운영/마스터**  | **Azure SQL**        | 사용자 정보, 센터 구역 ID, 라인 배치 정보, 시간당 임금 설정 | ACID 트랜잭션 보장 (데이터 누락/오류 절대 불가) |
| **실시간 캐시**  | **Azure Redis**      | 현재 구역별 Load(%), 실시간 병목 알람, 5분 단위 손실액  | 초저지연 대시보드 업데이트 (UI 성능 극대화)     |
| **실시간 분석**  | **Fabric KQL**       | 최근 1시간 센서 추이, 병목 현상 로그, 실시간 텔레메트리     | 대량 로그 데이터에 대한 고속 다차원 분석        |
| **빅데이터/AI** | **Fabric Lakehouse** | 1년치 센서 이력, 과거 병목 사례, AI 예지 정비 모델용 데이터 | 저비용 대용량 저장 및 Spark 기반 분산 처리    |

* * *

### 최종 요약 추천

1. **로컬 개발:** Docker + Kafka + Local SQL Server (비용 0원)

2. **클라우드 배포:**
   
   * **메시지:** Azure Event Hubs (Standard)
   
   * **운영/설정:** Azure SQL Database (Serverless)
   
   * **실시간 KPI:** Azure Cache for Redis (Basic)
   
   * **대용량 분석/AI:** Microsoft Fabric (F-SKU)

### **백엔드 서버 인프라 선정: Azure VM B2s**

**1. 선정 사양**
* **모델**: Azure B2s (Burstable 시리즈)
* **사양**: 2 vCPU, 4 GiB RAM, Windows OS
...
주의 (60-80%)   : 주황색 - 경고
위험 (80-95%)   : 빨강색 - 병목 
위험병목 (95%+)     : 어두운 빨강 - 즉시 조치 필요


3. 각 단계별 액션
정상: 그대로 운영
주의: "부하 증가 추세" 알림만 표시
위험: "병목 임박" 메시지 + 속도 저하 권유
병목: "병목 상태 진입" + 처리 일시 중지 권유?
